        ...
        if (canAccess) {
            ...
            if (str.length == 2) {
                ...
                if ((scrollUnits & SCROLL_TYPE.HOUR.value) == SCROLL_TYPE.HOUR.value) {
                    ...
                    hour_pv.setData(hour);
                    LogUtils.d(TAG,"hour="+timeStr[0]+",int ="+Integer.parseInt(timeStr[0]));
                    if(type==1){
                        if((Integer.parseInt(timeStr[0])+4)<10){
                            hour_pv.setSelected("0"+(Integer.parseInt(timeStr[0])+4));
                        }else {
                            hour_pv.setSelected((Integer.parseInt(timeStr[0])+4)+"");
                        }
                        selectedCalender.set(Calendar.HOUR_OF_DAY, Integer.parseInt(timeStr[0])+4);
                    }else {
                        if((Integer.parseInt(timeStr[0])+2)<10){
                            hour_pv.setSelected("0"+(Integer.parseInt(timeStr[0])+2));
                        }else {
                            hour_pv.setSelected((Integer.parseInt(timeStr[0])+2)+"");
                        }
                        selectedCalender.set(Calendar.HOUR_OF_DAY, Integer.parseInt(timeStr[0])+2);
                    }
